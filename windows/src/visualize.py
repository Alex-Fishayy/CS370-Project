...
def draw_face(frame, track_id, bbox, result):
    ...
    sub = f"y{result.yaw:+.0f} p{result.pitch:+.0f} ear{result.ear:.2f} g({result.gaze_x:+.2f})"
    ...
    face_w = x2 - x1
    arrow_len = max(20, face_w // 3)
    cx = (x1 + x2) // 2
    cy = (y1 + y2) // 2
    gx = int(cx + result.gaze_x * arrow_len)
    gy = int(cy + result.gaze_y * arrow_len)
    gaze_color = GREEN if result.gaze_ok else RED
    cv2.arrowedLine(frame, (cx, cy), (gx, gy), gaze_color, 2, tipLength=0.35)
